...
- An unreadable `recipes.yml` no longer leaves the loaded recipe list null. Recipe loading returns null when the file cannot be read or has no `recipes` section, and that null was stored as-is. It was harmless while nothing consulted the list, but the list is now read on every interaction, so it would have thrown once per right-click instead of once at startup. The list stays empty in that case.
...
- The plugin now reports a `startup` usage event on enable to the author's trace server so it is known which plugins are in use. The event carries the plugin name, the event name and the plugin version; nothing about players or the server. Medieval Cookery has no commands, so there is no per-command event. Reporting runs off the main thread, never delays a tick, drops silently when the server is unreachable, and is turned off with `usage-reporting.enabled: false` in the new `config.yml`. The default config carries the plugin's key, so reporting is active out of the box unless turned off — including on servers upgraded from a version before `config.yml` existed, whose data folder is never rewritten: the plugin reads the bundled defaults for any key the file lacks.

- The foods can be eaten. Right-clicking one held in the main hand starts a meal that takes 32 ticks, the same as a vanilla food, with the eating sound playing throughout; the food is then taken from the hand, the Saturation effect lasts for the recipe's `hungerDecrease` ticks, and the `afterEatItem` is returned. Crafting was previously the only behaviour the plugin had: the foods are player heads, which vanilla does not treat as edible, so no consumption event was ever fired for them and the eating code had no caller. The right-click is cancelled, so a food is no longer placed as a block instead of eaten. A meal is cancelled, with nothing consumed and no effect applied, if the food leaves the main hand before the last bite. A block with a use of its own — a chest, a door, a workstation — keeps its right-click, as it does for a player carrying a vanilla food, so carrying a food does not make the world unusable; sneaking eats in front of one.
- `CustomFoodItemTest` and `EatListenerTest`, which cover what may begin a meal: that a left-click, a pressure plate, an ordinary food, a block, a mob head and a right-clicked container are each turned away before anything is consumed.
